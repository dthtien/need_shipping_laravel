						...
						<script>
							...
							<div id="frmtt" class="s5h-userform">
								<div class="s5h-uformbox">
									<div class="s5h-uform">
										<form action="<?php echo route('postcapnhat'); ?>" method="post">
										...
										<?php if(count($errors) > 0): ?>
											...
											<div class="form-group has-feedback">
												<p class="s5h-uform-title">Họ tên:</p>
													<input type="text" id="show" class="form-control" style="" title="Click để chỉnh sửa thông tin" onclick="hien()" name="Name" value="<?php echo e(Auth::user()->name); ?>" readonly required>
												</div>
												<div class="form-group has-feedback">
													<p class="s5h-uform-title">Số Điện Thoại:</p>
														<input type="text" id="show1" title="Click để chỉnh sửa thông tin" class="form-control" onclick="hien()" name="Phone" value="<?php echo e(Auth::user()->sdt); ?>" readonly required>
													</div>
													<div class="form-group has-feedback">
														<p class="s5h-uform-title">Email: </p>
															<input type="email" class="form-control" title="Click để chỉnh sửa thông tin" onclick="hien()"  name="Email" value="<?php echo e(Auth::user()->email); ?>" readonly required>
														</div>
														<div class="form-group has-feedback">
															<p class="s5h-uform-title">Địa chỉ: </p>
																<input type="text" class="form-control" title="Click để chỉnh sửa thông tin" id="show3" onclick="hien()"  name="Address" value="<?php echo e(Auth::user()->diachi); ?>" readonly required>
															</div>
															<div class="form-group has-feedback">
															<p class="s5h-uform-title">Bạn là: </p>
																<input type="text" class="form-control" title="Click để chỉnh sửa thông tin" id="show4" onclick="hien()"  name="TypeAcc" value="<?php echo e(Auth::user()->loai); ?>" readonly required>
															</div>
															...
															<div id="myModal" class="modal fade" role="dialog">
															  ...
															  <form class="form-horizontal" action="<?php echo route('postcapnhatmatkhau'); ?>" method="post">
															  <input name="_token" type="hidden" value="<?php echo e(csrf_token()); ?>">
															    <!-- Modal content-->
